...
Current checkpoint: **Phase 1, Milestone 1.6 - implemented and verified with mocked provider responses**.

Ruff formatting/lint passed and the user reported **89 tests passed** in WSL.
Live PostgreSQL/API checks confirmed 202 upload scheduling, a no-evidence COMPLETED
result with provider `deterministic`, and an evidence-bearing FAILED result when
LLM configuration was absent. Statistics and selected evidence survived the failure;
analysis remained null and the API returned a safe error message. Successful
evidence-backed provider analysis and invalid output/citations were exercised with
mocks. No live OpenAI request was made; real provider compatibility/account access
remain unverified. Milestone 1.7 has not started.

Milestone 1.6 adds `app/analysis_schemas.py`, `app/services/llm_service.py`,
`app/services/pipeline.py`, `app/prompts/incident_analysis_v1.txt`, and mocked tests
in `tests/test_pipeline.py`. The upload router, settings, dependencies, and existing
upload tests are updated. Existing database columns and enum values are reused;
no migration or destructive table operation is introduced. `.env` is untouched.

### Current API behavior

`POST /api/v1/incidents` now returns **202 Accepted** with the saved incident's
UPLOADED snapshot. The task runs after the HTTP response. Poll
`GET /api/v1/incidents/{id}` for PARSING, CALCULATING, GENERATING_ANALYSIS,
VALIDATING, and then COMPLETED or FAILED. Fast stages may finish between polls.
Existing records are not automatically reprocessed. No retry API is introduced yet.

The background function receives the incident ID, session factory, and settings,
never the request's session or upload handle. It claims only UPLOADED records with
a conditional update, opens its own session, and commits meaningful status changes.
It reads the stored file, redacts/parses it, calculates statistics, selects evidence,
requests structured analysis, validates the result and citations, and stores it.
Errors close/roll back the working session and use a fresh session to record FAILED
with a generic safe message. If PostgreSQL itself is unavailable, even recording
FAILED may be impossible; only a safe message is logged and the old status may remain.

`statistics` now includes the deterministic fields plus an `evidence` list. Each
item includes its ID, redacted normalized event, and selection reasons. `analysis`
contains `result` (the structured explanation), `provider`, `model`, `prompt_version`,
`duration_seconds`, and `usage` when available. Statistics/evidence survive provider
or validation failures. Internal stored file paths remain excluded from API responses.

Structured analysis requires severity, summary, incident type, affected components,
observations, possible causes with confidence 0-1, diagnostic checks with risk labels,
information gaps, and an optional suggested escalation team. Every observation/cause
requires citations that must exist in the selected evidence. Extra fields are rejected.
Known secret patterns in provider output are masked before persistence and the result
is revalidated. This does not establish factual correctness or guarantee privacy.

No evidence candidates means no provider call: a deterministic COMPLETED result
states insufficient evidence, with empty observations/causes. It does not declare
the service healthy. This also allows a no-credit live smoke test.

### Provider setup and verification

Install the new SDK dependency in the active WSL environment:

```bash
python -m pip install -r requirements-dev.txt
```

Configure `OPENAI_API_KEY` and `LLM_MODEL` privately for a live model call.
No model is hardcoded. `LLM_MODEL` takes precedence over the older `OPENAI_MODEL`;
the latter remains a fallback for compatibility. Choose a model available to your
account that supports Responses structured outputs. Missing configuration produces
a controlled FAILED incident when evidence requires an LLM, not a startup failure.
`LLM_TIMEOUT_SECONDS` defaults to 60 per request. The SDK is configured for at most
two retries of retryable failures; schema/evidence failures are not loop-retried.
Total elapsed time may exceed one timeout due to retries. Provider input is bounded
to 200,000 UTF-8 bytes of selected statistics/evidence and output to 4,000 tokens;
oversized input, refusal, or incomplete output becomes a controlled failure.

Provider requests contain only statistics and redacted selected evidence. The
system prompt treats all payload text as untrusted, requires possible-cause language,
citations, and information gaps. No tools are supplied, and suggested commands are
never executed. `store=False` is requested; it is not a guarantee of zero provider
retention. Follow provider data policies and use synthetic data only.

Run Ruff formatting, lint, and Pytest using the existing commands, one guided command
at a time. Default tests block construction of a real OpenAI client and explicitly
mock the provider/SDK. Pipeline tests execute actual background scheduling using
isolated SQLite sessions; upload-only tests replace the task to isolate file behavior.
TestClient waits for tasks to finish before returning, unlike a real HTTP client.
No test needs live credentials or API credits. Live provider verification will be
requested separately; do not paste credentials or raw provider errors.

Manual verification covered the no-evidence COMPLETED case and controlled
configuration failure. A live evidence-backed provider check is optional and requires
private configuration and explicit approval to use API credits. Startup, readiness,
and existing upload validation behavior remain relevant.

### Concepts and limitations

**BackgroundTasks** schedules in-process work after the response; it is not a durable
queue. Restart/reload can lose pending tasks and leave in-progress states. A duplicate
claim is avoided, but exactly-once provider execution is not guaranteed. Celery and
recovery handling belong to later phases. Use one development worker for now.

**Structured output** describes the JSON contract to the provider with a Pydantic
model. Pydantic validates shape and ranges; application code separately validates
evidence IDs. Neither can prove a possible cause is true. **Provider isolation**
keeps SDK calls in one module so tests substitute a fake implementation.
**Session ownership** prevents a task from using a request session that may already
be closed. Commit status changes so other requests can observe them.

The single-file raw upload remains on disk. Redaction is conservative, not guaranteed.
There is no authentication, durable queue, retry endpoint, or automatic remediation.
Suggested shell/SQL commands remain strings for human review. Real provider calls
incur account usage once valid credentials/model are configured and evidence exists.

Suggested commit: `feat: add structured incident analysis and background pipeline`.

Milestone 1.6 interview questions:

1. Why return 202? The upload is saved but analysis has not necessarily finished.
2. Why a separate task session? Request resources have a different lifetime.
3. Why validate evidence after Pydantic? Valid JSON can still cite nonexistent IDs.
4. Why mock the provider? Tests stay deterministic, offline, and free of API charges.
5. Why is BackgroundTasks unsuitable for durable work? Jobs live in the API process
   and can be lost on restart; a separate durable queue is needed later.

Reference: [OpenAI structured outputs](https://developers.openai.com/api/docs/guides/structured-outputs/).

## Previous milestone notes (historical behavior)

The sections below record earlier checkpoints. The 202 background workflow above
supersedes their 201/no-analysis behavior and deferred-pipeline descriptions.
